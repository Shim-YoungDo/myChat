...
io.sockets.on('connection', function(socket) {
    ...
    socket.on('logout', function(logout){
        ...
        console.log('접속한 클라이언트 ID 갯수 : %d', Object.keys(login_ids).length);
        console.log("접속한 클라이언트 ID : ", Object.keys(login_ids));
        	...
        	if (message.command == 'chat') {
                ...
                if(roomList.length>0){
                    for(var i =0;i<roomList.length; i++){
                        if(roomList[i].id != message.recepient){
                            sendResponse(socket, 'error', '404','[' + message.recepient + ']대화방을 찾을 수 없습니다.');
                            return;
                        }
                    }
                } else{
                    sendResponse(socket, 'error', '404','대화방이 존재하지 않습니다.');
                    return;
                }
    ...
    socket.on('room', function(room) {
        ...
        if (room.command === 'create') {
        	...
        	if (io.sockets.adapter.rooms[room.roomId]) { // 방이 이미 만들어져 있는 경우
                ...
                sendResponse(socket, '','','방이 이미 만들어져 있습니다.');
